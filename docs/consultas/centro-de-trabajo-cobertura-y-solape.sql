-- ===========================================================================
-- CENTRO DE TRABAJO (work_center) -- invariantes de la serie temporal
-- ===========================================================================
-- Serie de: el empleado. Una ocurrencia vigente cada vez (ADR-057, decision 0).
-- Cobertura: OBLIGATORIA -- comprobado sobre mil empleados: los datos ya la
--            cumplian (ADR-057, decision 1).
--
-- Comprueba dos invariantes, y los dos por separado:
--   SOLAPE  dos asignaciones del mismo empleado vigentes el mismo dia.
--   HUECO   un tramo de presencia sin ninguna asignacion vigente.
--
...
WITH ocurrencia AS (
    SELECT wc.employee_id,
           wc.id,
           wc.start_date,
           wc.end_date,
           daterange(wc.start_date, COALESCE(wc.end_date, DATE '9999-12-31') + 1, '[)') AS vigencia
    FROM employee.work_center wc
),
presencia AS (
...
SELECT 'HUECO',
       ...
       e.employee_type_code,
       e.employee_number,
       format('presencia %s (%s..%s) sin centro de trabajo en %s',
              p.presence_number,
