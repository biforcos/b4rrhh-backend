-- ===========================================================================
-- CLASIFICACION LABORAL (labor_classification) -- invariantes de la serie
-- ===========================================================================
-- Serie de: el empleado. Una ocurrencia vigente cada vez (ADR-057, decision 0).
-- Cobertura: OBLIGATORIA -- de ella salen las tablas de convenio que el
--            calculo necesita (ADR-057, decision 1).
--
-- Comprueba dos invariantes, y los dos por separado:
--   SOLAPE  dos clasificaciones del mismo empleado vigentes el mismo dia.
--   HUECO   un tramo de presencia sin ninguna clasificacion vigente.
--
...
WITH ocurrencia AS (
    SELECT l.employee_id,
           ...
           l.start_date,
           l.end_date,
           daterange(l.start_date, COALESCE(l.end_date, DATE '9999-12-31') + 1, '[)') AS vigencia
...
SELECT 'HUECO',
       ...
       e.employee_type_code,
       e.employee_number,
       format('presencia %s (%s..%s) sin clasificacion en %s',
              p.presence_number,
